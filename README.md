...
| `PLAN.md` | 学習プラン（公式23章マップ）、進捗、次アクション | 学習開始前と節目ごと |
| `PROGRESS.md` | 現在の章・次の一手（中断/復帰の基準） | 毎セッション開始時 |
| `RECOVERY.md` | 中断・スリープ・再起動からの復帰手順 | 環境が止まった/再起動時 |
| `Specification.md` | 環境構築の正本、設計理由、構成の根拠 | 環境変更前・詳細確認時 |

中断・復帰のクイックリファレンス

突然のスリープ・再起動・Copilot 終了からでも、次の合言葉で再開できます。

```bash
# ターミナルを開いて
cd ~/Documents/Rails     # 必ずリポジトリルート（ここで AGENTS.md の家庭教師ペルソナが自動適用）
make resume              # アプリ起動 + HTTP 200 確認 + 再開手順を表示
copilot --continue       # 直近の学習セッションを再開（または: copilot --resume / copilot）
```

詳細な手順（スリープ復帰・Mac再起動・Docker復旧・最終手段の初期化）は `RECOVERY.md` を参照。
