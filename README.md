...
| ファイル | 役割 | 使い方 |
|---|---|---|
| `.github/copilot-instructions.md` | リポジトリ全体の Copilot 指示 | 自動適用 |
| `.github/instructions/rails-tutorial.instructions.md` | `blog/` 配下の Rails 学習コード向け指示 | 対象ファイルで自動適用 |
| `.github/instructions/docs.instructions.md` | README / PLAN / Specification / Copilot 関連文書の整合指示 | 対象ファイルで自動適用 |
| `.github/agents/rails-tutor.agent.md` | Rails を教えるためのカスタム agent | VS Code で `rails-tutor` agent を選択 |
| `.github/prompts/rails-tutorial.prompt.md` | Rails チュートリアル継続用の prompt file | VS Code Copilot Chat で `/rails-tutorial` |

この構成は、GitHub 公式の **repository instructions / path-specific instructions / prompt files / custom agents** の考え方に合わせています。  
使い分けは次の通りです。

- **自動で効かせる方針**: `.github/copilot-instructions.md`
- **Rails 学習コードや文書だけに絞る方針**: `.github/instructions/*.instructions.md`
- **「Ruby on Rails の先生」ペルソナを固定する**: `rails-tutor.agent.md`
- **毎回同じ学習プロンプトを再利用する**: `/rails-tutorial`
