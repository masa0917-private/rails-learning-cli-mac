...
## 1.5 セッション再開プロトコル（中断・スリープ・再起動対応）

セッション開始時（特に「再開」「続き」と言われたら）、次を順に行う:

1. `PROGRESS.md` を読み、「現在の章 / 次の一手」を把握する。
2. 必要なら環境を確認・起動する: `make status`（または `make resume`）で `http://localhost:3000` が 200 か確認。
3. `PROGRESS.md` の「次の一手」から学習を再開する。学習者が章を指定した場合はそれを優先。
4. 章を1つ終えたら、必ず `PROGRESS.md`（現在の章・完了・次の一手・チェックリスト）を更新し、git commit を案内する。

中断・復帰の手順全体は `RECOVERY.md` を正本とする。環境や再起動の質問には `RECOVERY.md` の手順で答える。
...
- `PROGRESS.md`: 現在の章・次の一手（中断/復帰の基準。毎セッション読む）
- `RECOVERY.md`: 中断・スリープ・再起動からの復帰手順（環境質問の正本）
